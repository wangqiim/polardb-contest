...
在该设置下，每页16条记，有246页，不刷脏页的情况下一共可以存储3936条记录。
`log_buffer_mgr.hpp`
```h
// 调参: 本地运行需要调小
const uint64_t PmemSize = 0.01 * (1024ULL * 1024ULL * 1024ULL); // 16G 搓搓有余
// ------ log_buffer_mgr.hpp -------
...
const uint64_t LogBufferBodySize = 272ULL * 256ULL  * 0.25 * 0.25; // (272 * 256) LogBufferSize = LCM(272, 256), 必须是倍数(log buffer需要判断是否满)
